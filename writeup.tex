...
\begin{table}[H]
    ...
    \begin{tabular}{cccc}
        ...
        \midrule
        8  & \TODO{$N$} & \TODO{mean} & \TODO{std} \\
        16 & \TODO{$N$} & \TODO{mean} & \TODO{std} \\
        32 & \TODO{$N$} & \TODO{mean} & \TODO{std} \\
        \bottomrule
    \end{tabular}
    \caption{Forward-pass timing for a ViT with $d_{\mathrm{model}}=384$, 6 heads, 6 blocks, batch size 16, averaged over 20 measured steps after 5 warmup steps.}
    \label{tab:vit-patch-size}
\end{table}

\TODO{Write a 2--3 sentence discussion. Include how attention cost changes as $P$ shrinks and when smaller patches are worth the extra compute.}
...
\TODO{Write 1--2 sentences explaining why the CLIP loss is symmetric, i.e. why it averages image-to-text and text-to-image cross-entropy.}

\subsection{EuroSAT Pretraining}

\begin{figure}[H]
    \centering
    \fbox{\parbox[c][1.7in][c]{0.8\linewidth}{\centering \TODO{Insert training-loss curve.}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][1.7in][c]{0.8\linewidth}{\centering \TODO{Insert zero-shot validation accuracy curve.}}}
    \caption{Zero-shot validation accuracy during CLIP pretraining on EuroSAT.}
    \label{fig:clip-val-acc}
\end{figure}

\TODO{Write 2--3 sentences on how training loss and validation accuracy relate. Mention whether training loss keeps improving after validation accuracy plateaus.}
